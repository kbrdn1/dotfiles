...
`aside exec` prints `created new session: <id>`. Continue with `--session <id>` (kept ~15 min after exit).
`aside repl` dies with the process. Run `aside repl` in a TTY so thinking/tool output streams live.
