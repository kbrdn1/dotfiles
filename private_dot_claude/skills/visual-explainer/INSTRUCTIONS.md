...
**What aesthetic?** Always use the **Claude Dark** palette. Read `./references/claude-dark-palette.md` before every generation — it contains the mandatory CSS variables, Mermaid theme variables, and both dark/light theme values. Do not improvise colors.

Within that palette constraint, vary the **layout and typography** to keep things fresh:
- Editorial (serif headlines, generous whitespace)
- Blueprint (technical drawing feel, grid lines, precise)
- Paper/ink (warm tones from the palette, hand-drawn Mermaid mode)
- Hand-drawn / sketch (Mermaid `handDrawn` mode, wiggly lines)
- Data-dense (small type, tight spacing, maximum information)
- Glassmorphism (blur, semi-transparent surfaces using palette surface colors)

The swap test still applies: if you replaced your fonts and layout with a generic template and nobody noticed, push further on typography and spatial design.
...
**For the color palette**, always read `./references/claude-dark-palette.md` first. Copy the CSS template verbatim.
...
**Color is Claude Dark.** Always use the CSS template from `./references/claude-dark-palette.md`. Copy the full `:root` block and `@media (prefers-color-scheme: light)` override verbatim. Do not invent custom palettes. The palette provides all the variables you need: `--bg`, `--surface`, `--surface-0/1/2`, `--border`, `--text`, `--text-dim`, `--text-muted`, `--accent`, `--accent-dark`, `--accent-dim`, plus semantic colors (`--success`, `--warning`, `--error`, `--info`, `--special`, `--secondary`) each with a `-dim` variant for backgrounds. Dark-first: `:root` is Pure Dark, light override in the media query.
