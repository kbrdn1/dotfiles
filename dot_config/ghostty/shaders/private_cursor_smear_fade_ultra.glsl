...
// Claude Dark Theme - Orange Accent (#D4825D)
const vec4 TRAIL_COLOR = vec4(0.831, 0.510, 0.365, 1.0);
const float DURATION = 0.5; //IN SECONDS

// 🚀 ULTRA-OPTIMIZED: Single sample only for maximum performance
const float BLUR_SAMPLES = 1.0; // ULTRA: Was 3.0, now 1.0 = NO BLUR LOOP
// 🚀 ULTRA-OPTIMIZED: Minimal blur intensity
const float BLUR_INTENSITY = 0.3; // ULTRA: Was 0.5, now 0.3
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // 🚀 ULTRA: Motion blur DISABLED (loop only runs once with BLUR_SAMPLES = 1)
    // This essentially removes the blur effect while keeping the trail
    if(lineLength > 0.001) {
        vec4 blurAccum = vec4(0.0);

        // This loop only executes ONCE now
        for(float i = 0.0; i < BLUR_SAMPLES; i++) {
            float t = i / max(BLUR_SAMPLES - 1.0, 1.0); // Avoid division by zero
        ...
        // Apply motion blur (minimal with single sample)
