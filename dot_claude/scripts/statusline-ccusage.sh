...
# Get dynamic colors based on usage
if type get_cost_color &>/dev/null; then
  session_cost_color=$(get_cost_color "$session_cost")
  daily_cost_color=$(get_cost_color "$daily_cost")
  block_cost_color=$(get_cost_color "$block_cost")
else
  session_cost_color=$LIGHT_GRAY
  daily_cost_color=$LIGHT_GRAY
  block_cost_color=$LIGHT_GRAY
fi

# Get token color (assuming max 200K tokens)
if type get_token_color &>/dev/null; then
  token_color=$(get_token_color "$session_tokens" 200000)
else
  token_color=$LIGHT_GRAY
fi

# Get time color
if type get_time_color &>/dev/null && [ "$remaining_minutes" != "0" ] && [ -n "$remaining_minutes" ]; then
  time_color=$(get_time_color "$remaining_minutes")
else
  time_color=$LIGHT_GRAY
fi

# Build the first line: branch / style / folder / model (with orange model)
first_line="${LIGHT_GRAY}🌿 $branch ${GRAY}|${LIGHT_GRAY} 💄 $output_style ${GRAY}|${LIGHT_GRAY} 📁 $dir_path ${GRAY}|${ORANGE} 🤖 $model_name${RESET}"

# Build the second line: pricing and tokens with dynamic colors
session_cost_display="${session_cost_color}\$$formatted_session_cost${RESET}"
if [ -n "$session_duration" ]; then
    session_cost_display="$session_cost_display ${GRAY}($session_duration)${RESET}"
fi

second_line="${LIGHT_GRAY}💰 ${RESET}$session_cost_display ${GRAY}|${LIGHT_GRAY} 📅 ${daily_cost_color}\$$formatted_daily_cost${RESET} ${GRAY}|${LIGHT_GRAY} 🧊 ${block_cost_color}\$$formatted_block_cost${RESET}"

if [ "$remaining_time" != "N/A" ]; then
    second_line="$second_line ${GRAY}(${time_color}${remaining_time} left${RESET}${GRAY})${RESET}"
fi

second_line="$second_line ${GRAY}|${LIGHT_GRAY} 🧩 ${token_color}${formatted_tokens}${RESET} ${GRAY}tokens${RESET}"
